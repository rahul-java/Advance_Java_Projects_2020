...
public class UserDao {
	...
	public boolean fatchAdminId(String name, String pass) throws SQLException {
		...
		PreparedStatement ps = con.prepareStatement("select uid,pwd from admin where uid=?  and pwd=?");
		ps.setString(1, name);
		ps.setString(2, pass);
	...
	public static boolean insertUserRegister(Registration registration) throws SQLException {
		boolean check= false;
		Connection con = Database_connection.getconnection();
		PreparedStatement ps = con.prepareStatement("insert into userregister (citizenName,address,dob,fname,contactN,occupation,gender) values(?,?,?,?,?,?,?)");
